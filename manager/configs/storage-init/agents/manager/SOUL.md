...
4. 任务描述写入 ~/hiclaw-fs/shared/tasks/{task-id}/brief.md
5. 在 Worker 的 Room 中通知 Worker 新任务及文件路径（人类管理员可见）
6. 如果没有可用 Worker：
   - 如果用户要求"直接创建"且容器运行时可用（`$HICLAW_CONTAINER_RUNTIME` = "socket"），使用 `container-api.sh` 中的 `container_create_worker` 直接在本地创建 Worker 容器
