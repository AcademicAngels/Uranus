...
Pre-configured MCP server YAML templates are stored in `/opt/hiclaw/configs/mcp-*.yaml`. These templates define the server metadata, credentials placeholder, and all available tools.
...
For servers with YAML templates in `/opt/hiclaw/configs/`:

```bash
# 1. Read the YAML template and substitute credentials
MCP_YAML=$(sed 's|accessToken: ""|accessToken: "'"${HICLAW_GITHUB_TOKEN}"'"|' /opt/hiclaw/configs/mcp-github.yaml)
...
- **Template re-use**: To add a new MCP server type, add its YAML to `/opt/hiclaw/configs/mcp-<name>.yaml` and follow the same pattern
